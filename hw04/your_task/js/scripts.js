...
const getTracks = (term) => {
  ...
  //1. go out and fetch the tracks associated with the search term:
  const url = 'https://www.apitutor.org/spotify/simple/v1/search?type=track&q=' + term;
  //2. go out and fetch the tracks associated with the url, and them dump the resulting data to the screen:
  fetch(url)
    ...
    .then(data => {
      ...
      for (const track of data){
        ...
        // B. Figure out which element in DOM to target
        document.querySelector('#tracks').innerHTML+= template;
        ++counter;
      }

      // after the for loop has completed and the DOM has been populated, we need to add the click event handlers to play the song:
